...
项目计划采用 `AGPL-3.0-only + 替代商业许可`。为保留双授权能力，外部代码、文档、设计和翻译贡献在合并前必须通过未来启用并经验证的 CLA 流程；当前尚未接受这类版权性贡献。

当前 [`CLA.md`](./CLA.md) 是准备启用的个人 CLA 候选文本，计划被授权方为 **郝春霖 (Hao Chunlin)**，但仍为 `NOT ACTIVE`。在联系邮箱、协议专业复核、公共仓库、CLA Assistant 和分支必需检查完成复核与测试前，不得合并外部版权性贡献。仅在 PR 文本中勾选一句话、评论或发送邮件，不能替代有效且可追踪的签署记录。

启用后，贡献者应只通过 CLA Assistant 展示的正式协议完成签署，仓库以 `license/cla` 状态作为合并依据。需要提交由公司或其他组织所有的材料时，应先联系项目；个人 CLA 不当然覆盖组织权利，当前也尚未开放公司 CLA。候选协议的中文解释见[个人 CLA 中文说明](./docs/legal/CLA_GUIDE.zh-CN.md)，签署数据范围见[CLA 隐私说明](./docs/legal/CLA_PRIVACY_NOTICE.md)。
