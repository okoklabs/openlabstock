...
const fullRisk = paths.some((entry) => (
  ...
  || entry === 'public/sw.js'
  || /^scripts\/(?:backup|handoff|instance-agent|instance-registry|release|release-version|prepare-release|next-version|release-smoke|verify|verification-state|verify-auto|check-public-boundary)\.mjs$/i.test(entry)
