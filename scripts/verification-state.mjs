...
function collect(relativePath, rootDir, { allowMissing = false } = {}) {
  const absolutePath = path.join(rootDir, relativePath);
  // `dist/` is generated by the verification build and is intentionally ignored by Git.
  // A clean checkout therefore has no directory to hash until the build step runs.
  if (!existsSync(absolutePath)) {
    if (allowMissing) return [];
    throw new Error(`Verification input is missing: ${relativePath}`);
  }
  ...
  return readdirSync(absolutePath, { withFileTypes: true }).flatMap((entry) => {
    ...
    if (entry.isDirectory()) return collect(child, rootDir, { allowMissing });
...
export function verificationState(rootDir) {
  const files = [...new Set(verificationRoots.flatMap((entry) => collect(entry, rootDir, {
    allowMissing: entry === 'dist',
  })))].sort();
