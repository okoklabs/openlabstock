...
test('登录、二维码定位与确认登记只产生一次库存写入', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('.summary-card').filter({ hasText: '库存预警' })).toBeVisible();
  await expect(page.locator('[data-filter-select="stock"] option[value="low"]')).toHaveText('库存预警');
