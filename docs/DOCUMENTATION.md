...
1. [`DEPLOYMENT.md`](../DEPLOYMENT.md) 与 [`deploy/openlabstock.sh`](../deploy/openlabstock.sh)：固定 Release 的自动下载、校验、首次安装、更新、备份和回滚入口。
2. [`deploy/docker/README.md`](../deploy/docker/README.md)：可复用 Docker 部署流程。
3. [`deploy/windows/README.md`](../deploy/windows/README.md)：Windows 双击运维向导与 SSH 密码边界。
4. [`deploy/PRODUCTION.md`](../deploy/PRODUCTION.md)：发布包运行约定。
5. [`QR_CODE_WORKFLOW.md`](./QR_CODE_WORKFLOW.md) 与 [`PWA_MOBILE_APP.md`](./PWA_MOBILE_APP.md)：专项部署条件和验收。
